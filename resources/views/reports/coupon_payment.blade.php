                        ...
                        <div class="panel-filled">
                            ...
                            <div class="tabs-container">
                                <ul class="nav nav-tabs">
                                    <li class="active"><a data-toggle="tab" href="#tab-bonds" aria-expanded="false">Bonds</a></li>
                                        ...
                                        <div class="panel-body">
                                            <table class="table datatables">
                                              <thead>
                                                ...
                                                <th>Coupon Payable</th>
                                                <th>Action</th>
                                              </thead>

                                              <tbody>

                                                @foreach($coupon_payment as $cp)
                                                <tr>
                                                    <td>{{ $cp->user->username ?? '-' }}</td>
                                                    {{-- <td>{{ $cp->GLIDCredit }}</td> --}}
                                                    <td>{{ $cp->Security }}</td>
                                                    <td>{{ number_format($cp->Position, 2) }}</td>
                                                    <td>{{ \Carbon\Carbon::parse($cp->NextCouponDate)->toFormattedDateString() }}</td>
                                                    <td>{{ number_format($cp->CouponPayable, 2) }}</td>
                                                    <td>
                                                        <form action="{{ route('post-coupon') }}" method="post">
                                                            {{ csrf_field() }}
                                                            <input type="hidden" name="CouponPaymentRef" value="{{ $cp->CouponPaymentRef }}">
                                            ...
                                            <table class="table datatables">
                                              ...
                                              <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{ number_format($cp->Position, 2) }}</td>
                                                    <td>{{ \Carbon\Carbon::parse($cp->NextCouponDate)->toFormattedDateString() }}</td>
                                                    <td>{{ number_format($cp->CouponPayable) }}</td>
                                                    <td>
                                                        <form action="{{ route('post-coupon') }}" method="post">
                                                            {{ csrf_field() }}
                                                            <input type="hidden" name="CouponPaymentRef" value="{{ $cp->CouponPaymentRef }}">
